...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const container = document.getElementById('event-steps-container');
        const addBtn = document.getElementById('add-event-step');
        ...
        addBtn.addEventListener('click', function(e) {
            e.preventDefault();
            const allSteps = container.querySelectorAll('.event-step');
            const lastStep = allSteps[allSteps.length - 1];
            const newStep = lastStep.cloneNode(true);

            // Kosongkan input di clone
            newStep.querySelectorAll('input, select').forEach(el => {
            ...
            newStep.querySelector('.remove-event-step').classList.remove('d-none');

            container.insertBefore(newStep, addBtn.parentElement);
            updateRemoveButtons();
            reinitializeFlatpickr();
        });

        container.addEventListener('click', function(e) {
            if (e.target.matches('.remove-event-step')) {
                e.preventDefault();
                e.target.closest('.event-step').remove();
                updateRemoveButtons();
            }
        });

        function updateRemoveButtons() {
            const steps = container.querySelectorAll('.event-step');
            steps.forEach((step, idx) => {
                const removeBtn = step.querySelector('.remove-event-step');
                if (steps.length > 1) {
