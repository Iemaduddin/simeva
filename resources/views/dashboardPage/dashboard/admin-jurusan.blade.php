    ...
    <div class="row gy-4">
                    ...
                    <div
                        ...
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-1 mb-8">
                            <div>
                                ...
                                <h6 class="fw-semibold mb-1"> {{ $totalAsset }} Aset</h6>
                            </div>
                            <span
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td></td>
                                        <td></td>
                                        <td class="text-start">Tidak ada peminjaman aset dalam waktu dekat
                                            ini.
                                        </td>
                                    </tr>
                                @endforelse
        ...
        <div class="col-xxl-12">
            <div class="card h-100">
                <div class="card-header">
                    <div class="d-flex align-items-center flex-wrap gap-2 justify-content-between">
                        <h6 class="mb-2 fw-bold text-lg mb-0">Penggunaan Aset Fasilitas Jurusan {{ $kode_jurusan }}</h6>
        ...
        <div class="col-xxl-12">
            <div class="card h-100 p-24">
                <div class="card-header text-center">
                    <h6>Jadwal Penggunaan Aset Jurusan {{ $kode_jurusan }}</h6>
                </div>
                <div class="card-body">
                    <div class="row gy-4">
                        <div class="col-xl-12">
                            ...
                            @endphp
                            <div id="timeline-usage-asset" data-default-scope="{{ 'jurusan_' . $jurusanId }}"
...
@push('script')
    ...
    <script>
        const role = "{{ $role ?? '-' }}"
        const kodeJurusan = "{{ $kode_jurusan ?? '-' }}";

        const urlTemplate = "{{ route('dash.admJur.getStatusAssetBookingJurusanChart', ['kode_jurusan' => '__KODE__']) }}";
        const finalUrl = urlTemplate.replace('__KODE__', kodeJurusan);
        fetch(finalUrl)
    ...
    <script>
        ...
        function renderAssetChart(yearFilter) {
            const routeTemplate =
                `{{ route('dash.kaurPU.getUsageAssetJurusanChart', ['kode_jurusan' => '__KODE_JURUSAN__']) }}`;
            const kodeJurusan = '{{ $kode_jurusan ?? '' }}';

            const finalUrl = routeTemplate.replace('__KODE_JURUSAN__', kodeJurusan) + `?year=${yearFilter}`;
            fetch(finalUrl)
                .then(res => res.json())
                .then(data => {
                    const options = {
