  ...
  <ul class="pagination mt-40 flex-align gap-12 flex-wrap justify-content-center" id="showing-text">
      ...
      <li class="page-item {{ $events->onFirstPage() ? 'disabled' : '' }}">
          <a class="page-link text-neutral-700 fw-semibold w-40 h-40 bg-main-25 rounded-circle hover-bg-main-600 border-neutral-30 hover-border-main-600 hover-text-white flex-center p-0"
              href="#" data-page="{{ $events->currentPage() - 1 }}">
      ...
      @for ($i = 1; $i <= $events->lastPage(); $i++)
          <li class="page-item {{ $i == $events->currentPage() ? 'active' : '' }}">
              <a class="page-link {{ $i == $events->currentPage() ? 'bg-main-600 text-white' : 'text-neutral-700' }} fw-semibold w-40 h-40 bg-main-25 rounded-circle hover-bg-main-600 border-neutral-30 hover-border-main-600 hover-text-white flex-center p-0"
                  href="#" data-page="{{ $i }}">
                  {{ $i }}
              </a>
          </li>
